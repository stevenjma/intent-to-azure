...
/**
 * POST a git-data write, retrying on 404. A just-created repo's git backend can
 * briefly 404 write endpoints before it's fully provisioned; a few short retries
 * ride that out. Non-404 errors surface immediately.
 */
 ...
 *
 * We deliberately do NOT use `auto_init`: building both the base commit and the
 * feature branch by hand avoids the replication race where git-data writes 404
 * against a freshly auto-initialized repo, and still gives the PR a real diff.
 *
...
export async function createRepoAndPush(repoName, isPrivate, scaffoldFiles, commitMessage) {
  ...
  // 1. Base branch: a README-only initial commit (no parents) → refs/heads/base.
  //    This creates the repo's default branch under our own control (no race).
  const readme = `# ${name}\n\nAzure infrastructure generated by azx.\n`;
  const baseTree = await gitWrite(`${R}/git/trees`, {
    tree: [{ path: "README.md", mode: "100644", type: "blob", sha: await blobSha(readme) }],
  });
  const baseCommit = await gitWrite(`${R}/git/commits`, {
    message: "azx: initialize repository",
    tree: baseTree.sha,
    parents: [],
  });
  await gitWrite(`${R}/git/refs`, { ref: `refs/heads/${base}`, sha: baseCommit.sha });
  ...
  const tree = await gitWrite(`${R}/git/trees`, { base_tree: baseTree.sha, tree: treeItems });
  ...
  // 3. Open the PR into the default branch.
  const pr = await gh(`${R}/pulls`, {
    method: "POST",
    body: {
