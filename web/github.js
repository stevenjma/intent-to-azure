...
/** Sleep helper for the brief auto_init settle window. */
const sleep = (ms) => new Promise((r) => setTimeout(r, ms));

/**
 * Fetch the head SHA of `branch`, retrying briefly: right after `auto_init`,
 * GitHub creates the initial commit asynchronously, so the ref may 404 for a
 * moment. Give it a few short attempts before giving up.
 */
async function headShaWithRetry(owner, name, branch) {
  let lastErr;
  for (let i = 0; i < 5; i++) {
    try {
      const ref = await gh(`/repos/${owner}/${name}/git/ref/heads/${branch}`);
  ...
  throw lastErr || new Error(`Could not read ${branch} head after repo creation.`);
}
...
export async function createRepoAndPush(name, isPrivate, scaffoldFiles, commitMessage) {
  ...
  const baseCommit = await gh(`/repos/${owner}/${name}/git/commits/${headSha}`);

  // 1. Blobs for every scaffold file.
  const treeItems = [];
  ...
  // 2. Tree layered on the initial tree (keeps auto_init's README) →
  //    3. commit parented on the initial commit → 4. push to azx-infra branch.
  ...
  const commit = await gh(`/repos/${owner}/${name}/git/commits`, {
    method: "POST",
    body: { message: commitMessage || "azx: infra scaffold", tree: tree.sha, parents: [headSha] },
  });
  ...
  // 5. Open the PR into the default branch.
