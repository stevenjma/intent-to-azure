...
export async function createRepoAndPush(repoName, isPrivate, scaffoldFiles, commitMessage) {
  ...
  // 1. Base branch: seed a README initial commit via the Contents API. This is
  //    the reliable way to create the first commit in an empty repo — it makes
  //    the branch + commit atomically and avoids the git-data "Git Repository is
  //    empty" 409 you get writing blobs/trees to an unborn repo.
  ...
  const baseCommitSha = seed.commit.sha;
