...
/** Render untrusted text as a single-line Bicep string literal. */
export function bicepString(value: string): string {
...
function bicepComment(value: string): string {
  return value.replace(/[\r\n\u2028\u2029]/g, " ");
}

function assertResourceShape(r: AzureResource): void {
  if (!/^Microsoft\.[A-Za-z0-9.]+\/[A-Za-z0-9]+(?:\/[A-Za-z0-9]+)*$/.test(r.type)) {
    throw new Error(`unsafe ARM resource type: ${JSON.stringify(r.type)}`);
  }
}
...
export function generateBicep(plan: AzurePlan): string {
  ...
  for (const r of plan.resources) {
    if (idToSymbol.has(r.id)) throw new Error(`duplicate Azure resource id: ${JSON.stringify(r.id)}`);
    let symbol = symbolFor(r.id);
    if (usedSymbols.has(symbol)) symbol = `${symbol}${shortHash(r.id)}`;
    usedSymbols.add(symbol);
    idToSymbol.set(r.id, symbol);
  ...
  out.push(`// Resolved region: ${bicepComment(plan.region)}`);
  if (plan.budget) {
    out.push(
      `// Advisory estimate: ~$${bicepComment(String(plan.budget.estimatedMonthlyUsd))}/mo ` +
        `${bicepComment(plan.budget.currency)}; usage charges may vary.`,
    );
  }
  out.push("// ---------------------------------------------------------------------------");
  out.push("");
  ...
  out.push(`param location string = ${bicepString(plan.region)}`);
  out.push("");
...
function skuBlock(r: AzureResource): string[] | undefined {
  switch (r.type) {
    case "Microsoft.DBforPostgreSQL/flexibleServers": {
      ...
      return ["sku: {", `  name: ${bicepString(sku)}`, `  tier: ${bicepString(tier)}`, "}"];
    }
    case "Microsoft.CognitiveServices/accounts":
      return ["sku: {", `  name: ${bicepString(r.sku ?? "S0")}`, "}"];
...
function propertiesBlock(r: AzureResource, idToSymbol: Map<string, string>): string[] | undefined {
  switch (r.type) {
    ...
    case "Microsoft.App/containerApps": {
      ...
      const minReplicas = numDeep(r, ["scale", "minReplicas"], 0);
      const maxReplicas = numDeep(r, ["scale", "maxReplicas"], 3);
    ...
    case "Microsoft.DBforPostgreSQL/flexibleServers/configurations": {
      ...
      return ["properties: {", `  value: ${bicepString(value)}`, `  source: ${bicepString(source)}`, "}"];
    }
    case "Microsoft.CognitiveServices/accounts":
      return [
        "properties: {",
        "  publicNetworkAccess: 'Enabled'",
        `  customSubDomainName: ${bicepString(r.name)}`,
        "}",
      ];
    case "Microsoft.CognitiveServices/accounts/deployments": {
      ...
      return [
        ...
        "  model: {",
        "    format: 'OpenAI'",
        `    name: ${bicepString(model)}`,
        ...(version ? [`    version: ${bicepString(version)}`] : []),
        "  }",
        "}",
      ];
    }
