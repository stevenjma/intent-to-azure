/**
 * Stage [0] scan-core — browser-safe capability detection over an in-memory file
 * map. This is the SINGLE SOURCE OF TRUTH for signal detection: both the Node CLI
 * (`read-repo.ts`, which walks the real filesystem) and the browser SPA (which
 * fetches files via the GitHub API) build a `Map<string,string>` of repo-relative
 * path -> text content and hand it to `scanFileMap`. No `node:` imports live here,
 * so this module bundles cleanly for the web.
 *
 * Paths are always repo-relative and forward-slashed. Callers are responsible for
 * pre-filtering to text files and capping per-file size before building the map.
 */
...
/** basename() for a forward-slashed repo-relative path (no node:path needed). */
...
/**
 * A cached, filtered view of a repository's text files, backed by an in-memory
 * map rather than the filesystem. Same read/has/first/match surface the detectors
 * expect, so the detection logic below is identical across CLI and browser.
 */
class RepoIndex {
  ...
  constructor(fileMap: Map<string, string>) {
    ...
    this.files = [...fileMap.keys()].sort((a, b) => a.localeCompare(b));
  }

  has(rel: string): boolean {
    return this.fileMap.has(rel);
  }
  ...
  /** Read a repo-relative file. Returns undefined if absent. */
  read(rel: string): string | undefined {
    return this.fileMap.get(rel);
  }
...
/** Collapse control chars + whitespace to a single safe display line, capped. */
...
/**
 * Run every detector over an in-memory file map and return the app info + raw
 * signals (stage 0). `appName` seeds `app.name`/`app.root` (a detector such as
 * `detectNode` may refine `app.name` from package.json).
 */
export function scanFileMap(appName: string, files: Map<string, string>): RepoScan {
  const idx = new RepoIndex(files);
  const signals: Signal[] = [];
  const push = (s: Signal) => signals.push(s);

  const safeName = cleanAppName(appName) || "app";
  const app: AppInfo = { name: safeName, root: safeName };
